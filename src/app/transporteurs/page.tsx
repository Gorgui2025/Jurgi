...
function TransporteursContent() {
  const searchParams = useSearchParams();
  const initialQuery = searchParams.get("q") || "";

  const [profiles, setProfiles] = useState<TransporterProfile[]>([]);
  ...
  const fetchProfiles = async (pageNum: number, append: boolean) => {
    ...
    try {
      const res = await fetch(`/api/transporter-profiles?${params.toString()}`);
      const data = await res.json();
      const items: TransporterProfile[] = data.profiles || [];
  ...
  return (
    <div className="page-container">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-charbon-500">Annuaire des transporteurs</h1>
        <p className="text-sm text-charbon-300">
          Trouvez un transporteur près de chez vous au Sénégal
        </p>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-charbon-300">
          {loading
            ? "Chargement..."
            : `${total} transporteur${total !== 1 ? "s" : ""} trouvé${total !== 1 ? "s" : ""}`}
        </p>
      </div>
      ...
      {loading ? (
        <div className="text-center py-16">
          ...
          <p className="text-charbon-300 text-sm">Recherche des transporteurs...</p>
        </div>
      ) : profiles.length === 0 ? (
        <div className="text-center py-16">
          <Truck className="w-12 h-12 text-charbon-200 mx-auto mb-3" />
          <p className="text-charbon-400 font-medium text-lg mb-2">Aucun transporteur trouvé</p>
        ...
        <div className="flex flex-col gap-3 sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:gap-3">
          {profiles.map((profile) => {
            ...
            const acceptedTypes = parseAcceptedTypes(profile.acceptedTypes);
            const zones = parseZones(profile.zones);
            const displayName = profile.displayName || profile.name || "Transporteur";
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 space-y-1.5 text-xs text-charbon-300">
                  ...
                  {profile.indicativePrice != null && (
                    <div className="flex items-center gap-1.5">
                      <Truck className="w-3.5 h-3.5 shrink-0" />
                      <span className="truncate">
                        Prix indicatif : {profile.indicativePrice.toLocaleString()} FCFA
                      </span>
                    </div>
                  )}
                </div>
      ...
      {!loading && !hasMore && profiles.length > 0 && (
        <div className="text-center mt-6">
          <p className="text-xs text-charbon-200">
            Tous les transporteurs ont été affichés
          </p>
        </div>
      )}
    </div>
